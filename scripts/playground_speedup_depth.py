...
def analyze_case(n: int, depth: int, cutoff: float, angle_scale: float,
    ...
    speedup       = (cmp["sv_total"]/cmp["hybrid_total"]) if cmp["hybrid_total"] > 0 else np.inf
    return {
        "sv_total_norm": sv_total_norm,
...
def main():
    ap = argparse.ArgumentParser(
        description="Depth sweep playground: plot speedup (SV/Hybrid) vs depth for given n and cutoff lists."
    )
    ...
    ap.add_argument("--target-speedup", type=float, default=1.0,
                    help="Reference speedup line (SV/Hybrid). Example: 1.25 for 25%% faster.")
    ...
    ap.add_argument("--save-json", type=str, default=None,
                    help="If set, save full sweep results to this JSON for later analysis.")
    args = ap.parse_args()
    ...
    for c in args.cutoff:
        for n in args.n:
            spds, sv_norms, hyb_norms = [], [], []
            for d in depths:
                res = analyze_case(n, d, c, args.angle_scale, est)
                spds.append(res["speedup_vs_sv"])
    ...
    plt.ylabel("Speedup (SV_cost / Hybrid_cost)")
    plt.title(f"Speedup vs depth | conv={args.conv_factor}, twoq={args.twoq_factor}, angle_scale={args.angle_scale}")
