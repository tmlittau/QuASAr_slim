...
def _best_whole_baseline(baselines: Dict[str, Any]) -> Optional[Tuple[str, float]]:
    ...
    def is_whole(e: Dict[str, Any]) -> bool:
        if e.get("ok") is False: return False
        if e.get("scope") in ("whole","global","circuit"): return True
        if e.get("per_partition") is False: return True
        if "partition_id" not in e and "chain_id" not in e: return True
        return False
    best = None
    for e in entries:
        if not is_whole(e): continue
        t = e.get("wall_s_measured"); t = e.get("wall_s_estimated") if t is None else t
...
def _parse_reason_norms(reason: str) -> Dict[str, float]:
    out = {}
    if not isinstance(reason, str): return out
    try:
...
def make_plot(suite_dir: str, out: Optional[str] = None, title: Optional[str] = None):
    ...
    for data in cases:
        ...
        n_qubits = int(metrics.get("num_qubits", n))
